...
package freechips.rocketchip.rocket  //将本文件打包成整体，命名为：freechips.rocketchip.rocket
...
/*
 *FrontendReq：请求流水线前端的信号类
 *pc：当前inst对应地址；speculative表示是否进行推测执行；
*/
...
/*
 *FrontendExceptions：异常处理
 *pf：prefetche；ae：addressextension
*/
...
/*
 *FrontendResp：流水线前端的应答信号类型
 *btb：分支处理的信息；pc：ID stage PC；data：all the inst of one packet；
 *mask：切分packet取出单独的inst；xcpt：Frontend exception;replay:回复FrontendReq的处理状态；
*/
...
/*
 *PerfEvent=Performance Event记录，用于软件性能分析的工具;
 *acquire：获取pc状态；tlbMiss：TLB中miss状态；
 *cache miss=num(acquire)-num(tlbMiss)?
*/
...
/*
 *FrontendIO：定义前端外延出的io口信息；
 *might_request：
 *clock_enabled:时钟使能;
 *req:前端请求信号；
 *sfence：superviosr memory-management fence instruction;
 *resp：frontend的响应情况；
 *btb_update<-前端因处理branch/jump(from Execute stage)/Backing Predictor(from Fetch stages)而引起的redirected
 *bht_update<-the branch is fetched and predicted;
 *ras_update(eg. call)<-the Fetch stages once the instructions in the Fetch Packet have been decoded;
 *flush_icache:刷新icache，会在reset阶段或者mispredicted出现；
 *npc：next pc;
 *perf:Performance Event记录；
*/
class FrontendIO(implicit p: Parameters) extends CoreBundle()(p) {
  val might_request = Bool(OUTPUT)
  val clock_enabled = Bool(INPUT)
  val req = Valid(new FrontendReq)
  val sfence = Valid(new SFenceReq)  //sfence?
  val resp = Decoupled(new FrontendResp).flip
  val btb_update = Valid(new BTBUpdate)
  val bht_update = Valid(new BHTUpdate)
  val ras_update = Valid(new RASUpdate)
  val flush_icache = Bool(OUTPUT)  //flush_icache~clear
  val npc = UInt(INPUT, width = vaddrBitsExtended)
  val perf = new FrontendPerfEvents().asInput  
}4

/*
 *Frontend:定义整个前端的基本情况；
 *module<-FrontendModule;
 *icache配置为LazyModule，其中hartid：CPU ID;
 *masterNode/slaveNode:主从节点，applied for bus interface；
*/
class Frontend(val icacheParams: ICacheParams, hartid: Int)(implicit p: Parameters) extends LazyModule {
  ...
  val masterNode = icache.masterNode  //applied for bus interface 
  val slaveNode = icache.slaveNode
}

/*
 *FrontendBundle：流水线前端的对外连线；
 *cpu<-FrontendIO类型；
 *ptw：硬件页表查询；
 *errors：统计ICacheErrors；
*/
class FrontendBundle(val outer: Frontend) extends CoreBundle()(outer.p)  //outer.p?
    with HasExternallyDrivenTileConstants {
  val cpu = new FrontendIO().flip  //.flip means cpu is a DFF?
  val ptw = new TLBPTWIO()
  val errors = new ICacheErrors
}

/*
 *FrontendModule:前端实现的重点代码，基于LazyModule；
 *io<-FrontendBundle;
 *edge：流水线的触发方式；
*/
@chiselName
class FrontendModule(outer: Frontend) extends LazyModuleImp(outer)
  ...
  implicit val edge = outer.masterNode.edges.out(0)  //流水线的触发方式；
  val icache = outer.icache.module
// require要求一次性获取指令的字节数等于I-cache一次性获取的字节数；
  require(fetchWidth*coreInstBytes == outer.icacheParams.fetchBytes)
//fq：fetch queue的模式为一次可容纳5条inst的ShiftQueue；
  val fq = withReset(reset || io.cpu.req.valid) { Module(new ShiftQueue(new FrontendResp, 5, flow = true)) }

  val clock_en_reg = Reg(Bool())  
  val clock_en = clock_en_reg || io.cpu.might_request  //clock有效<-clock_en_reg/cpu.might_request；
  io.cpu.clock_enabled := clock_en  //将clock_en锁进latch里，并在下个cycle赋值给cpu.clock_enabled；
  ...
  val gated_clock =  //gated_clock门控时钟，外接系统时钟clock；
...
//tlb<-TLB;fetchBytes == 4; nTLBEntries == 32；
  val tlb = Module(new TLB(true, log2Ceil(fetchBytes), TLBConfig(nTLBEntries)))

//s0:verify the validation of the cpu/fq;
//NOTE:s0不是reg类型;
//reg s1: npc generation;
//reg s2:icache/BTB/BP(GSHARE)/fetch queue;
  val s1_valid = Reg(Bool())
  val s2_valid = RegInit(false.B)   //s2 initialise with the value 0；
  val s0_fq_has_space =             //check whether the space of fq is full.
 ...
 //s0_valid:在cpu接受到req或者fq时候满位，使能s0_valid;
 //以下定义各阶段涉及到的信号；
  val s0_valid = io.cpu.req.valid || s0_fq_has_space  
  s1_valid := s0_valid  //wait for the next cycle;
  val s1_pc = Reg(UInt(width=vaddrBitsExtended))
  val s1_speculative = Reg(Bool())  //推测s1阶段是否执行；
  val s2_pc = RegInit(t = UInt(width = vaddrBitsExtended), alignPC(io.reset_vector))  
  val s2_btb_resp_valid = if (usingBTB) Reg(Bool()) else false.B  //s2推测是用于BTB，经过BTB是判断taken的inst
  val s2_btb_resp_bits = Reg(new BTBResp)  //resp后发送的bits是预测inst的pcy以及taken?
  ...
  val s2_partial_insn_valid = RegInit(false.B)  //dynamic branch prediction use partial decoding;
  val s2_partial_insn = Reg(UInt(width = coreInstBits))
  val wrong_path = Reg(Bool())

  val s1_base_pc = ~(~s1_pc | (fetchBytes - 1))  //s1基地址，当前指令说对应的pc;
  val ntpc = s1_base_pc + fetchBytes.U  //正常的下一条指令所在pc；
  val predicted_npc = Wire(init = ntpc) //分支预测得到的predicted_npc,wire类型，初试值为ntpc；
  val predicted_taken = Wire(init = Bool(false))  //BHT预测结果，T/N；

  val s2_replay = Wire(Bool())//s2阶段重载<-s2_valid有效，fq仍存在inst；
  s2_replay := (s2_valid && !fq.io.enq.fire()) || RegNext(s2_replay && !s0_valid, true.B)
//npc为下一条inst所在地址，其中s2_replay：s2重载说对应地址/s2_pc：正常指向程序的下一条inst所在地址/predicted_npc：预测分支所在地址；
  val npc = Mux(s2_replay, s2_pc, predicted_npc)  
  s1_pc := io.cpu.npc 

//consider RVC fetches across blocks to be non-speculative if the first；
//part was non-speculative；
//s0_speculative赋值；当使用压缩模块，对各有效位进行判断/正常模式下，s0_speculative=1；
  val s0_speculative =
...
//s2_redirect，pc重定位；
  val s2_redirect = Wire(init = io.cpu.req.valid)  //初始化时，s2_redirect=cpu.req.valid；
  s2_valid := false    //初始化时，s2_valid置0；
  when (!s2_replay) {  //s2不需要replay时，配置s2_valid/s2_pc/s2_speculative/s2_tlb_resp；
...
//配置btb，包含BTB/BHT/RAS；
    ...
    when (!s2_replay) {
// 如果不回滚，则不需要进行重定位，表示按照原顺序执行 或者 预测的Target PC正确
      btb.io.req.valid := !s2_redirect  
      s2_btb_resp_valid := btb.io.resp.valid
      s2_btb_resp_bits := btb.io.resp.bits
    }
//预测有效且inst为taken状态->将预测得到的分支赋值给predicted_npc且predicted_taken=1；
    when (btb.io.resp.valid && btb.io.resp.bits.taken) {
      predicted_npc := btb.io.resp.bits.target.sextTo(vaddrBitsExtended)  //predicted_npc<-将分支程序的首个inst的地址;
      predicted_taken := Bool(true)
    }

    val force_taken = io.ptw.customCSRs.bpmStatic  //ptw(page table walker):硬件查询页表；
    ...
    val useRAS = Wire(init=false.B)  //出现jump/call此类指令，需要RAS；
    val updateBTB = Wire(init=false.B)

//以下属于区分指令中的return/Call/Branch/JAL/Imm，主要分为RVI和RVC类；
    def scanInsns(idx: Int, prevValid: Bool, prevBits: UInt, prevTaken: Bool): Bool = {  
      def insnIsRVC(bits: UInt) = bits(1,0) =/= 3      // 指令的机器码的[1:0] == 11表示普通指令，不是11就是压缩指令
      val prevRVI = prevValid && !insnIsRVC(prevBits)  // 记录是否是普通指令
      val valid = fq.io.enq.bits.mask(idx) && !prevRVI
      val bits = fq.io.enq.bits.data(coreInstBits*(idx+1)-1, coreInstBits*idx)  // idx指向的指令的位
      val rvc = insnIsRVC(bits)  //判断指令是否为压缩指令；
      val rviBits = Cat(bits, prevBits)  // 获取预解码所需要的位域

// 具体指令对应的机器码，查看Instruction.scala文件
// BEQ，BNE，BLT，BGE，BLTU，BGEU指令的[6:0]位都相等（1100011），表示条件跳转指令
// JAL指令的[6:0]位                               （1101111）
// JALR指令的[6:0]位                              （1100111）
// Call指令为 直接跳转 或者 寄存器跳转
// Return指令为 寄存器跳转（就像ARM中使用的R14寄存器，用于保存函数的返回地址）（只有当指令是JALR指令，且[7] == 0，且[19,15]满足"b00?01"，则为Return指令）      
      ...
      val rviImm = Mux(rviBits(3), ImmGen(IMM_UJ, rviBits), ImmGen(IMM_SB, rviBits))  // 指令的[3]表示是否使用立即数
// rvc中的c表示compress，压缩指令，这些指令都只有16位   
      val rvcBranch = bits === Instructions.C_BEQZ || bits === Instructions.C_BNEZ  // 指令集中给出的两条压缩的条件跳转指令C_BEQZ/C_BNEZ
      val rvcJAL = Bool(xLen == 32) && bits === Instructions.C_JAL                  // 指令集中给出的JAL指令 [15:13]==001
      val rvcJump = bits === Instructions.C_J || rvcJAL                             // C_J指令              [15:13]==101 
      ...
      val rvcReturn = rvcJR && BitPat("b00?01") === bits(11,7)                       // Return指令首先一定是寄存器跳转指令，其次[11:7] == "b00?01"
      ...
      val taken =  //taken=1<-出现Jump或者JALR/被预测为taken的分支；
...
//s2阶段，btb预测到branch inst（rvi）->清空btb，replay
      ...
      when (!prevTaken) {  
        taken_idx := idx  //idx指fq中正在处理的inst
        after_idx := idx + 1
//ras更新的条件<-fq中是否有空位留出fq.io.enq.fire()&预测正确&出现Call/Return；
        btb.io.ras_update.valid := fq.io.enq.fire() && !wrong_path && (prevRVI && (rviCall || rviReturn) || valid && (rvcCall || rvcReturn))
        btb.io.ras_update.bits.cfiType := Mux(Mux(prevRVI, rviReturn, rvcReturn), CFIType.ret,  //firstly: 判断RV处于哪种模式下；rviReturn->CFIType.ret=3
                                          Mux(Mux(prevRVI, rviCall, rvcCall), CFIType.call,
                                          Mux(Mux(prevRVI, rviBranch, rvcBranch) && !force_taken, CFIType.branch,
                                          CFIType.jump)))

        when (!s2_btb_taken) {  //当s2阶段未处理inst时，使能wrong_path/useRAS/计算pc/npc->predicted_npc
          when (fq.io.enq.fire() && taken && !predictBranch && !predictJump && !predictReturn) {
            wrong_path := true
          }
          when (s2_valid && predictReturn) {  //s2阶段，出现return指令；
            useRAS := true
          }
          when (s2_valid && (predictBranch || predictJump)) {  //s2阶段，出现Branch/Jump指令；
            val pc = s2_base_pc | (idx*coreInstBytes)  //当前pc；
            val npc =                                  //npc<-分支指令的地址;
              if (idx == 0) pc.asSInt + Mux(prevRVI, rviImm -& 2.S, rvcImm)  //fq中当前处理的inst索引值；
              else Mux(prevRVI, pc - coreInstBytes, pc).asSInt + Mux(prevRVI, rviImm, rvcImm)
            predicted_npc := npc.asUInt
          }
        }
        when (prevRVI && rviBranch || valid && rvcBranch) {  //当fq中仍有空余位时，使能btb；
        ...
        when (!s2_btb_resp_valid && (predictBranch && s2_btb_resp_bits.bht.strongly_taken || predictJump || predictReturn)) {
          updateBTB := true  //s2阶段中，btb_resp_valid=0且预测的Branch中处于strongly_taken/Jump/Return，更新BTB；
        }
      }

      if (idx == fetchWidth-1) {  //当fq中inst被submit完毕后，s2_partial_insn？
        when (fq.io.enq.fire()) {
        ...
        scanInsns(idx + 1, valid, bits, prevTaken || taken)  //当fq中还有其他inst未被执行，再次查询该指令属于branch/jump/...；
      }
    }
//btb不更新时，不断查询btb状态;
    ...
    btb.io.ras_update.bits.returnAddr := s2_base_pc + (after_idx << log2Ceil(coreInstBytes))  //处理完分支指令后的返回地址

    val taken = scanInsns(0, s2_partial_insn_valid, s2_partial_insn, false.B)  //taken这个是指在执行中的inst->branch的taken状态
    when (useRAS) {
      predicted_npc := btb.io.ras_head.bits  //将ras_head存放的地址->predicted_npc
    }
    when (fq.io.enq.fire() && (s2_btb_taken || taken)) {  //taken后将s2_partial_insn_valid置0
      s2_partial_insn_valid := false
    }
    when (!s2_btb_taken) {  //当s2_btb_taken=0时
      when (taken) {
    ...
    when (s2_redirect) { s2_partial_insn_valid := false }  //redirect时，s2无效；
  ...
  clock_en_reg := !rocketParams.clockGate ||
    io.cpu.might_request || // chicken bit: A bit on a chip that can be used to disable one of the features of the chip if it proves faulty or negatively impacts performance. 
  ...
  def alignPC(pc: UInt) = ~(~pc | (coreInstBytes - 1))  //alignPC：地址对齐
