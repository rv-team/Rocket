...
/*
  Edit Author : wn
  Date        : 2019.8.12
  压缩指令的译码：先将所有的压缩指令放在s这个队列中，根据输入的16位指令的其中5位取选择s中的哪一组。
  RVCExpander在ibuf.scala中被调用，在RocketCore.scala中未被调用
*/
...
//共三个类，第三个类调用第二个类，第二个类调用第一个。第三个类调用第二个类
...
class RVCDecoder(x: UInt, xLen: Int) {  //x是指令，xlen是x的bit数
  def inst(bits: UInt, rd: UInt = x(11,7), rs1: UInt = x(19,15), rs2: UInt = x(24,20), rs3: UInt = x(31,27)) = {
    val res = Wire(new ExpandedInstruction)
    res.bits := bits //.bit应该是一个方法，取位宽
    res.rd := rd
    res.rs1 := rs1
    res.rs2 := rs2
    res.rs3 := rs3
    res //返回的 是(new ExpandedInstruction)接口都变成线性节点，即返回多个信号
  }
...
//返回的队列
  ...
  def q2 = {
    ...
    Seq(slli, fldsp, lwsp, flwsp, jalr, fsdsp, swsp, fswsp) //和q3一样输出8组inst的返回值
  }

  def q3 = Seq.fill(8)(passthrough) //输出矩阵吧，8组，每一组都有好几个信号

  def passthrough = inst(x) //此处x是 class RVCDecoder(x: UInt, xLen: Int) {
    // 是这个吗？ def inst(bits: UInt, rd: UInt = x(11,7), rs1: UInt = x(19,15), rs2: UInt = x(24,20), rs3: UInt = x(31,27)) = {


//q0,q1,q2,q3队列每个人都包含8组 每一组都是inst的返回信号

//q0,q1,q2,q3队列在这用上，在扩展指令
  def decode = {
    val s = q0 ++ q1 ++ q2 ++ q3
    s(Cat(x(1,0), x(15,13))) //此处x是 class RVCDecoder(x: UInt, xLen: Int) {
  }                 //取s这个队列的第几组，s共有32组，所以用x的其中五位表示
}

class RVCExpander(implicit val p: Parameters) extends Module with HasCoreParameters {
  val io = new Bundle {
    val in = UInt(INPUT, 32)
    val out = new ExpandedInstruction //class ExpandedInstruction里的每个信号都是out
    val rvc = Bool(OUTPUT)
  }

  if (usingCompressed) {
    io.rvc := io.in(1,0) =/= UInt(3) //低两位决定rvs是否是true
    io.out := new RVCDecoder(io.in, p(XLen)).decode //调用decode
  } else {
    io.rvc := Bool(false)
    io.out := new RVCDecoder(io.in, p(XLen)).passthrough //io.in是传入的指令（32位？）
  }                //passthrough = inst(x)，inst返回的多个信号，与io.out的多个信号相连。:=可以表示多组信号相连吗？
}
